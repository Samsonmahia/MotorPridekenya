class CarsDisplay {
    ...
    async loadCars() {
        try {
            ...
            if (!Array.isArray(data)) throw new Error('Invalid cars data');
    ...
    renderCars() {
        ...
        if (!grid) return console.error('❌ cars-grid element missing');
    ...
    openCarDetail(slug) {
        ...
        if (!modal || !content) return console.error('Modal missing');
    ...
    createCarDetailHTML(car) {
        const thumbnails = car.images?.slice(0, 7).map((img, i) =>
            `<img src="${img}" alt="${car.title}" 
                  class="thumb" onclick="document.getElementById('main-image').src='${img}'">`
        ).join('') || '';
        ...
        return `
            <div class="car-detail-header">
                ...
                <div class="car-detail-meta">
                    ...
                    <span class="status-badge status-${car.status?.toLowerCase() || 'available'}">${car.status?.toUpperCase() || 'AVAILABLE'}</span>
                </div>
                <div class="car-detail-price">${car.price}</div>
            </div>

            <div class="car-detail-gallery">
                <img id="main-image" src="${car.images?.[0] || '/images/cars/sample.jpg'}" alt="${car.title}">
            ...
            <div class="reserve-section">
                <button class="reserve-btn" onclick="reserveManager.openReserveModal()">🚗 Reserve This Vehicle</button>
            </div>`;
    }
